...
def ad_loss(semi_targets, dist, eta=1, eps=1e-6):
    """
    Compute losses based on semi_targets and dist.

    Args:
        semi_targets (torch.Tensor): Tensor of semi targets.
        dist (torch.Tensor): Tensor of distances.
        eta (float): A scaling factor.
    ...
    losses = torch.where(
                    semi_targets == 1,
                    (dist + eps) ** semi_targets.float(),
                    eta * ((dist + eps) ** semi_targets.float())
        )
    return 0.001*torch.mean(losses)
